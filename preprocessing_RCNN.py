...
def load_train_proposals(datafile, num_clss, save_path, threshold=0.5, is_svm=False, save=False):
    ...
    for num, line in enumerate(train_list):
        ...
        if save:
            np.savez(os.path.join(save_path, tmp[0].split('/')[-1].split('.')[0].strip()) + '_data.npz', images=images, labels=labels)
    print(' ')
    fr.close()
...
def load_from_npy(data_set):
    ...
    for ind, d in enumerate(data_list):
        data= np.load(os.path.join(data_set, d))
        i, l = data['images'], data['labels']
        images.extend(i)
        labels.extend(l)
